...
public final class AtlasLoadTable extends AbstractWidget
{
    ...
    public void renderWidget(GuiGraphics graphics, int mouseX, int mouseY, float partialTick)
    {
        ...
        int uOff = reverse ? 114 : 82;
        int vOff = reverse ? 5 : 20;
        graphics.pose().pushPose();
        graphics.pose().translate(colArrowX[sorting.ordinal()], getY() + 4, 0);
        graphics.pose().scale(.5F, .5F, 1F);
        graphics.blit(ARROW_LOCATION, 0, 0, uOff, vOff, 11, 7);
    ...
    public boolean mouseScrolled(double mouseX, double mouseY, double delta)
    {
        ...
        {
            if (delta < 0 && scrollOffset < atlasInfo.fillStats().size() - 4)
            {
                scrollOffset++;
            }
